...
****
A <<vol1_spec_sdpi_p_actor_somds_provider>> shall used the "preferred format", defined in  <<ref_rfc_9110, RFC9110 §5.6.7>>, for the `Date` field included in all HTTP response messages. 

.Notes
[NOTE]
[%collapsible]
====
* The `Date` field in HTTP response messages, as the best available approximation of the date and time of message generation, is mandatory for an origin server with a clock (<<ref_rfc_9110>>, §6.6.1) though several formats are supported. This clarifies obsolete formats are not be be used. 
