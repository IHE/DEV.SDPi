...
===== Scenario: <<acronym_stad>> {var_use_case_id}.7 - A <<vol1_spec_sdpi_p_actor_somds_consumer>>, operational in the MD LAN network, receives data affected by a non-slewing time adjustment
...
*Then* The <<vol1_spec_sdpi_p_actor_somds_consumer>> will create a log entry...  
