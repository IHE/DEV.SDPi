...
[%collapsible]
====
NOTE: The 50ms target accuracy is suitable for highly demanding use cases like real time waveform comparison.  Since <<acronym_ts_service>> configuration will only happen during the <<acronym_md_lan>> network initial setup, it is based on the worst case scenario.
====
****
