...
5+<|
Note 1: This actor initiates this transaction if the grouped actor from SDPi-P supports the *"export"* or *"bidirectional"* <<vol1_spec_sdpi_p_option_sdc_gateway_direction>>.
In this case, the gateway is performing a "consumer" role.

Note 2: This actor responds this transaction if the grouped actor from SDPi-P supports the *"import"* or *"bidirectional"* <<vol1_spec_sdpi_p_option_sdc_gateway_direction>>.
...
Gateway directionality (export, import, or bidirectional) is specified by the <<vol1_spec_sdpi_p_option_sdc_gateway_direction>> in the <<vol1_clause_sdpi_p_profile_reftext>>.
...
| SDC-to-DEC
| DOR
| Gateway retrieves information from <<vol1_spec_sdpi_r_actor_somds_medical_data_provider>> systems, converts and spublishing them as [DEV-01] / [PCD-01] Transactions to external DEC Device Observation Consumer (DOC) systems.
...
An actor from this profile (Column 1) shall implement all required transactions for the grouped actor (Column 3) in <<vol1_table_sdpi_r_required_actor_groupings>>.
