...
#**<add high-level description of DP option, including sequence diagram, requirements, rationale, behaviour, etc.>
**#
...
##David - how much of the information below is in the current TF-2 & TF-2A detail?  TBD whether the requirements should be here at a more generic level or there.  Theory of Operation and non-transaction-specific requirements should go in this section;
##
...
* IF a provider is unable to discover the desired consumers using the discovery proxy, then it may use the "ad hoc" DEV-24 to probe the presence of systems using multicast;
* ... #question# ... what about a given provider that has both proxy and "ad hoc" discovery-based consumer connections?  It will always update the proxy but is there anyway to know that a connection was made via "ad hoc" discovery means and not via the proxy? ...
