...
.^| <<transaction_name_manage_network_presence_metadata>>
.^| Initiator
.^| O   ^(See^ ^Note^ ^2)^
| <<vol2_clause_dev_46>>
...
| <<transaction_name_retrieve_network_presence_metadata>>
| Initiator
| O   ^(See^ ^Note^ ^2)^
| <<vol2_clause_dev_47>>
...
Note 2: Optional transaction is required if the SDPi-P <<vol1_spec_sdpi_p_option_discovery_proxy>> is enabled.  Some deployments may support a mix of systems that use the <<vol1_spec_sdpi_p_actor_somds_discovery_proxy>> Actor as well as the default "ad hoc" discovery mode.  Additional details and requirements are provided in the <<vol1_clause_sdpi_p_option_discovery_proxy>> discussion.
...
===== SOMDS Discovery Proxy

[#vol1_spec_sdpi_p_actor_somds_discovery_proxy, reftext='SOMDS Discovery Proxy']
Actor Summary Definition:
[none]
. Accepts presence and absence messages by <<vol1_spec_sdpi_p_actor_somds_provider>>s, sends heartbeats to known <<vol1_spec_sdpi_p_actor_somds_provider>>s, and responds to <<vol1_spec_sdpi_p_actor_somds_consumer>>s querying endpoint metadata.

This allows for a <<acronym_somds>> to provide centralized discovery of <<vol1_spec_sdpi_p_actor_somds_provider>>s in networks that cannot support decentralized discovery.
...
[#vol1_clause_sdpi_p_option_discovery_proxy]
==== Discovery Proxy Option
[#vol1_spec_sdpi_p_option_discovery_proxy, reftext='Discovery Proxy Option']

#**<add DEV-47 option for subscription / proxy "pass through"; integrate david's comments from PR thread; sync / refactor transaction names to David's PR comments; multicast / unsecure vs. secure man-in-the-middle messaging >
**#

Rationale:

* "Ad Hoc" discovery is the default way in which SOMDS_Participant Actors manage their network presence and departure;
* This default mode uses the DEV-23, DEV-24 and DEV-34 transactions, all of which rely on multicast services;
* Some network configurations do not support or allow multicast services, requiring an alternative means of discovery and network presence and departure management;
* The SOMDS Discovery Proxy Actor provides an alternative means for managing SOMDS_Provider Actor presence and departure from a network that does not require the use of multicast services;
* Some configurations may allow network configurations that simultaneously support both "ad hoc" (default) and proxy-based discovery

<<figure_sdpi-p-discovery-proxy-option-sequence-diagram>> provides an overview of the interactions of the SOMDS Consumer and Somes Provider Actors with the SOMDS Discovery Proxy Actor.

.SDPi-P Discovery Proxy Option - Transaction Sequence Diagram
[#figure_sdpi-p-discovery-proxy-option-sequence-diagram]
[plantuml, target=target=puml-sdpi-p-discovery-proxy-option-sequence-diagram, format=svg, format=svg]
....
include::../plantuml/vol1-figure-sdpi-p-discovery-proxy-option-sequence-diagram.puml[]
....


* SOMDS_Provider systems shall support use of the SOMDS_Discovery_Proxy Actor (DEV-46)
* SOMDS_Consumer systems shall support use of the SOMDS_Discovery_Proxy Actor (DEV-47)

#DESCRIBE DEV-46 usage + requirements#

* IF the Discovery Proxy Option is enabled for a SOMDS_Provider Actor, then it shall use the DEV-46 transaction to update the SOMDS Discovery Proxy Actor on its network presence and departure
* IF the Discovery Proxy Option is not enabled for a SOMDS_Provider, then it shall use the default "ad hoc" network presence transactions (DEV-23 -24 -34)
* if a SOMDS_Provider Actor is configured or provisioned for the Discovery Proxy Option, but the proxy system is not available, then the provider will default back to "ad hoc" discovery mode

#DESCRIBE DEV-47 usage + requirements#

* IF the Discovery Proxy Option is enabled for a SOMDS_Consumer Actor, then it shall use the DEV-47 transaction to retrieve the current network presence metadata related to those systems that it is looking to discover (see transaction metadata detail);
* IF the Discovery Proxy Option is not enabled for a SOMDS Consumer Actor, then it shall use the default "ad hoc" network presence transactions for discovery (DEV-23 -24 -34);
* if a SOMDS Consumer Actor is configured / provisioned for the Discovery Proxy Option, but the proxy system is not available, then the consumer will default back to "ad hoc" discovery mode

#DESCRIBE MIXED AD-HOC & PROXY-BASED Discovery + requirements#

##David:  If ad hoc and managed mode is used in parallel, a provider should send hellos (DEV-23) no matter if the discovery proxy is reachable or not.
##
* IF both ad-hoc discovery (using the DEV-23 - 24 - 34 transactions) and proxy-based discovery management (using DEV-46 -47 transactions) are utilized in a SOMDS network deployment ...
* Provider and Consumer systems shall prioritize use of the SOMDS Discovery Proxy Actor and related transactions;
* Provider systems shall only use the DEV-46 transaction to update the proxy system, and not use the DEV-23 or DEV-34 transactions;
* Consumer systems shall first use the DEV-47 transaction to determine the presence / absence of a given consumer or set of consumers;
* IF a consumer is unable to discover the desired providers using the discovery proxy, then it may use the "ad hoc" DEV-24 to probe the presence of systems using multicast;


NOTE:  Provisioning of SOMDS_Participant systems to support use of a SOMDS Discovery Proxy Actor is out-of-scope for this specification.
