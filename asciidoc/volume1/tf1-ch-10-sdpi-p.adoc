...
This foundational profile not only supports medical device interoperability ("<<acronym_mdi>>"), providing for “plug-and-play” capabilities, but also with a tightly integrated “trust” framework (see <<clause_p_n_t_interperability_requirements_management>>).
...
This required grouping ensures that all systems connecting to a SOMDS network support the <<acronym_ses_mdi>>  requirements footnote:[See <<clause_p_n_t_interperability_requirements_management>> and <<clause_ses_considerations_requirements>> for more detail on <<acronym_ses_mdi>>] necessary for establishing a <<term_plug_and_trust>> ecosystem, including the secure and dynamic provision of an implementation’s <<term_system_function_contribution>>.
See <<clause_requirements_from_narratives_to_pnt_interfaces>> for additional discussion.
