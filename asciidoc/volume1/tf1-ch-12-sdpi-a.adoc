...
This default configuration requires the grouped <<vol1_spec_sdpi_p_actor_somds_v2_gateway>> Actor <<vol1_spec_sdpi_p_option_sdc_gateway_direction>> (Direction="export") option is enabled.
...
In this case, the <<vol1_spec_sdpi_p_actor_somds_v2_gateway>> Actor  <<vol1_spec_sdpi_p_option_sdc_gateway_direction>> (Direction="bidirectional") is enabled.
...
An actor from this profile (Column 1) shall implement all required transactions for the grouped actor (Column 3) in <<vol1_table_sdpi_a_required_actor_groupings>>.
