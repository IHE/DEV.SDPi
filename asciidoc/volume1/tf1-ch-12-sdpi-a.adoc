...
This option will enable both the <<transaction_name_manage_medical_alert_delegation>> [DEV-41] and <<transaction_name_delegate_medical_alert>> [DEV-42] transactions, with the pre-requisites that also the <<vol1_clause_sdpi_a_actor_option_distributed_alarm_system>> option is selected and a Distributed Alarm System is active.
...
[cols="1"]
|===
a| *{supplement_note}*:  As indicated in <<vol1_figure_sdpi_a_actor_diagram>> above, there are four grouped actors:
