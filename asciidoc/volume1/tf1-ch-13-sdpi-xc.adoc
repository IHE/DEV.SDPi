...
Note that capability '_external or "remote" control of one device by another_' enabled this SDPi-xC Profile refers to "external" to the device that is controlled, while both devices are in the same secure environment (<<term_medical_device>>).
