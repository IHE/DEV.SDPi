...
//[sdpi_offset=2]
[#clause_sdpi_use_cases_60601_1_8_concepts_for_dis_das_cdas]
==== ISO/IEC 60601-1-8 Concepts for DIS, DAS and CDAS

The following is a quick guide to the functionality of DIS, CDIS, DAS and CDAS systems and how we have used and interpreted them for the purpose of this Gemini Profile Standard.
Please refer to the next section on Definitions from <<ref_iec_60601_1_8_2020>> for normative definitions for these terms.

NOTE: Note that the {gemini_standard_sdpi_revision_short} version of the SDPi Profile only supports the Distributed Information System (DIS) model detailed below.
...
For example, an anesthesia workstation, for which an OPERATOR is normally present during all PATIENT care, would not be expected to provide these functions.
