...
|
|
| See extended description and discussion in #LINK OR TEXT#:vol3_clause_system_type_nomenclature_extensions
|
...
| A BICEPS Participant Model extension that allows for a #LINK OR TEXT#:vol1_spec_sdpi_p_actor_somds_provider to provide attributes from the first partition of the IEEE 11073-10101 nomenclature. Specified in #LINK OR TEXT#:vol3_clause_coded_attribute.
|
|
...
| A set of zero to many identifiers that allows for organizing #LINK OR TEXT#:vol1_spec_sdpi_p_actor_somds_provider s into logical groups.
|
|
|
| SDC
...
| A service to access historical <<acronym_mdib>> data by means of an initial <<term_mdib_configuration>> and subsequent change reports. The History Service replaces the ARCHIVE SERVICE specified in <<acronym_biceps>> and is used to implement #LINK OR TEXT#:vol0_transaction_summary_dev_32.
|
|
|
| SDC
...
| Structured collection of any data objects that are provided by a #LINK OR TEXT#:vol1_spec_sdpi_p_actor_somds_provider or #LINK OR TEXT#:actor_biceps_content_creator, including both descriptive and state information.
|
...
| <<acronym_mdib>> that describes the state of a #LINK OR TEXT#:vol1_clause_sdpi_p_somds_provider at a specific MDIB version.
|
|
|
| SDC
...
| A general term that refers to all aspects of standards-based exchanges between medical (and health) devices, including <<acronym_pocd>> and <<acronym_phd>>; in some contexts, for example <<acronym_hl7>>, it refers to the IEEE 11073-10101 Nomenclature or "coding system".
|
| [[acronym_mdc,MDC]] MDC
...
| These generally refer to the IEEE 11073-1070x standards that provide a consensus set of risk control measures aligned with the four core <<acronym_mdi>> functions:  <<term_plug_and_trust>>, reporting, alerting and external control.
|
| [[acronym_pkp,PKP]] PKP
| ref_ieee_11073_10700_2022
| SDC
...
| A system that displays information from one or more #LINK OR TEXT#:vol1_spec_sdpi_p_actor_somds_participant systems associated with a single patient. Similar to a <<term_cockpit>> but without device-external control capabilities. May include both metric and alert information.
| Dashboard
|
|
|
...
| The source of time obtained from a <<term_time_synchronization_service>> and shared between #LINK OR TEXT#:vol1_spec_sdpi_p_actor_somds_participant s.
|
|
|
| Time
...
| A subsystem of a #LINK OR TEXT#:vol1_spec_sdpi_p_actor_somds_provider that can be attached to or removed from the #LINK OR TEXT#:vol1_spec_sdpi_p_actor_somds_provider and that is represented in the <<acronym_mdib>>.
|
|
...
| A globally unique identifier <<acronym_uid>> for a #LINK OR TEXT#:vol1_spec_sdpi_p_actor_somds_provider that is stable across re-initializations (i.e., resets / reboots).
| [[acronym_uid,UID]] UID
|
|
| SDC
...
| A source of <<term_timestamp>>s used in a #LINK OR TEXT#:vol1_spec_sdpi_p_actor_somds_participants system function contributions (<<acronym_sfc>>).
|
|
|
| Time
...
| Function of a #LINK OR TEXT#:vol1_spec_sdpi_p_actor_somds_participant that contributes to a <<term_clinical_function>> provided by a <<term_service_oriented_medical_device_system>>.
|
| [[acronym_sfc,SFC]] SFC
...
| A physical endpoint address that can be used to communicate with a #LINK OR TEXT#:vol1_spec_sdpi_p_actor_somds_provider.
| XAddr
|
|
|
