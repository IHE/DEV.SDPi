...
[cols="1"]
|===
a| *{standard_note}*: This standard is being written after the 2019 reorganization of the IHE Patient Care Devices (*PCD*) domain to the IHE Devices (*DEV*) domain.
...
// 2.2
[#clause_ses_considerations_requirements,sdpi_offset=2]
...
This standard extends these two concepts to include a third:  Effectiveness.
...
The background for <<acronym_ses>> is discussed in detail in <<clause_integrating_ses>>; however, in general "<<acronym_ses>>" is used as a reference to the standards encompassed (directly and indirectly by reference) in <<term_joint_working_group_7>>, including <<ref_iso_81001_1_2021>> and <<ref_iec_80001_1_2021>>.
...
Thus the "Safety, Effectiveness and Security - Requirements and Considerations" sections in this standard are intended to reflect the results of that risk management and to guide those who are tasked with deploying and managing these interoperable solutions during use.

Note that specific requirements from the above mentioned standards, may also be captured in <<clause_references_referenced_standards_conformance>>.
...
[cols="1"]
|===
a| *{standard_note}*: The template for this section assumes that it will be integrated with the technical framework section that is organized based on TF-1 section headings (e.g., chapter 1:10 for SDPi- would have a summary here as 1:2.3.10).  No provision is made, though, for general introductory sections such as the SDPi Overview & Framework discussion in 1:2.3.1 below.

In this version, the content is added as 1:2.3.1, and then the profiles as 1:2.3.10 to 1:2.3.13.  Though the content is valid, it may be repositioned in subsequent versions to better integrate with the IHE DEV TF at a future date.
...
The Service-oriented Device Point-of-care Interoperability (SDPi) Profile specifications provide detailed instructions for seamless plug-and-play interoperability between IEEE 11073 SDC-based medical devices (including SaMD), as well as between medical devices and health IT systems based on HL7 FHIR and HL7 Version 2.
...
.. For a more complete "Big Picture" perspective, see the discussion in <<clause_requirements_from_narratives_to_pnt_interfaces>>.
...
To that end, this specification implements the safety and security requirements of the <<ref_ieee_11073_10702_202x>> alert <<acronym_pkp>> standard (expected to be completed in 2026).
...
[cols="1"]
|===
a| *{standard_note}*:  The SDPi-xC Profile is provided for completeness and to show the general direction of the family of SDPi Profiles.
It is *_not part of the capabilities specified for SDPi {gemini_standard_sdpi_revision_short}_* and even basic controls will not be added until SDPi 3.0 or later.
|===
...
Given the significant risks associated with allowing device-external control functions in a network of <<acronym_pnt>> systems, this specification implements the safety and security requirements of the <<ref_ieee_11073_10703_202x>> external control <<acronym_pkp>> standard (in development, anticipated in 2027).


[sdpi_offset=5]
=== Dependencies between SDPi and Other IHE Integration Profiles

////
#TODO:  SHOULD ATNA BE ADDED TO THIS TABLE FOR SOMDS_PARTICIPANT?#
////

The following table identifies dependencies between the SDPi profiles in this standard and other IHE technical framework profiles.
"Dependency" indicates that implementation of an SDPi profile requires implementation of another external profile.

NOTE:  Implementation dependencies on other non-IHE profiles are addressed in the normative references section and other referenced standards clauses throughout this standard.

[#vol1_table_devices_integration_profile_dependencies]
.SDPi Dependencies on IHE Profiles
...
| The <<vol1_spec_sdpi_r_actor_somds_dec_gateway>> integrates DEC Device Observation Reporter (DOR) Actor specifications.
...
| The <<vol1_spec_sdpi_a_actor_somds_acm_gateway>> integrates ACM Alert Reporter (AR) Actor specifications.
