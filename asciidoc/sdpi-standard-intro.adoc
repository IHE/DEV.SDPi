...
This version of the SDPi 2.5 Standard for Trial Use (HL7 STU) / Trial Implementation (IHE TI) supplement is the 2nd release in 2026, continuing the objective to provide three to four releases each year that include both _incremental_ updates (e.g., editorial "fixes"), ballot comment resolutions (e.g., from HL7 ballot cycles), and additional capability enhancements.
For this release, key changes include:

1. Resolution of all May 2026 HL7 Ballot comments
2. New History Service option with related transaction DEV-32
3. Alignment with the SDC BICEPS standard corrigendum published in December 2025
4. Support for semantic markup for deprecating requirements, transactions, use-cases, profiles, content modules and actors
5. Initial two-document refactoring towards “Standalone SDPi” 

Numerous additional corrections and enhancements are included.
...
. *Requirements boxes* (e.g., "R1234") are included especially in TF-2, with some also part of TF-1 and TF-3; this is an initial approach that *_will be significantly expanded in future versions of the supplement_*; documentation is provided in <<vol1_clause_sdpi_requirements_modeling_integration>>, including discussion related to how it will be expanded in future versions of the supplement;
