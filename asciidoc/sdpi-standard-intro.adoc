:standard_note: SDPi {ihe_standard_sdpi_revision_short} Standard Note

[#standard_clause_introduction_to_standard,sdpi_offset=clear]
...
[cols="1"]
|===
a| *SDPi {ihe_standard_sdpi_revision_short} Standard -- _STU / TI Version_ -- Note*:

// STANDALONE TO DO ... this section


This version of the SDPi 2.3 Standard for Trial Use (HL7 STU) / Trial Implementation (IHE TI) standard is the 3rd release in 2025, continuing the objective to provide three to four releases each year that include both _incremental_ updates (e.g., editorial "fixes"), ballot comment resolutions (e.g., from HL7 ballot cycles), and additional capability enhancements.
...
3. Added semantic markup to support rich artifact generation
...
For a list of those related to this release, see <<sdpi_release_issue_management>> below.
...
NOTE: This standard also includes both normative and informative references to other specifications (see <<vol1_appendix_b_references>>), some of which are finalized and published and others that are either being developed or in revision.
...
NOTE: #Copyright and Trademark info#

include::volume0/tf0-ch-9-copyrights.adoc[]
...
[#standard_clause_sdpi_standard_overview]
== SDPi Standard Overview

[#standard_clause_sdpi_organization]
=== SDPi Organization
...
NOTE: #(content from TF-0, Forward declarations, ... )#

include::volume0/tf0-ch-d-glossary.adoc[]

NOTE: #FIX FOR BOGUS TRANSACTION ERRORS IN BUILD - NEED TO REFACTOR OUT#
include::volume0/tf0-ch-b-transactions.adoc[]


[#standard_clause_requirements_glossary]
