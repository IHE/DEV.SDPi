...
<<vol0_transaction_summary_dev_32>> leverages <<vol0_transaction_summary_dev_27>> to initiate a subscription, which is used to deliver historic <<acronym_mdib>> data to a <<vol1_spec_sdpi_p_actor_somds_consumer>>.
...
`s12:Envelope/s12:Body/wse:Subscribe/wse:Filter/hm:HistoryQuery/hm:ProvideInitialMdib`:: Set to `true` if an initial <<acronym_mdib>> is requested or `false` otherwise.

`s12:Envelope/s12:Body/wse:Subscribe/wse:Filter/hm:HistoryQuery/hm:LanguageFilter`:: A list of languages for localized texts to be requested (see data type description in <<vol2_clause_appendix_a_mdpws_history_service_xmlschema>> for more details).

`s12:Envelope/s12:Body/wse:Subscribe/wse:Filter/hm:HistoryQuery/hm:ReportFilter`:: An optional list of reports desired by the 
<<vol1_spec_sdpi_p_actor_somds_consumer>> (a consumer may choose not to request `WaveformStream` reports,  for example). 
A <<vol1_spec_sdpi_p_actor_somds_provider>> _may_ choose not to send
historic reports for `pm:AbstractReport`s that are not included in this list. The list is empty, or the element
is missing, when all reports are desired by the <<vol1_spec_sdpi_p_actor_somds_consumer>>.

`s12:Envelope/s12:Body/wse:Subscribe/wse:Filter/hm:HistoryQuery/hm:HandleFilter`:: An optional list of `pm:HandleRef`
containment tree entries desired by the <<vol1_spec_sdpi_p_actor_somds_consumer>>. 

* A <<vol1_spec_sdpi_p_actor_somds_provider>> _may_ choose to send historic `pm:AbstractReport`s only for 
referenced handles. 
* When the `pm:HandleRef` references an instance of a `pm:AbstractDeviceComponentDescriptor`
the instance and all its children are desired by the <<vol1_spec_sdpi_p_actor_somds_consumer>>.
* The list is empty, or the element is missing, when all reports are desired by the <<vol1_spec_sdpi_p_actor_somds_consumer>>.

`s12:Envelope/s12:Body/wse:Subscribe/wse:Filter/hm:HistoryQuery/hm:Range`:: A range that selects the historic <<acronym_mdib>> data, either based on time frames or <<acronym_mdib>> versions (see data type description in <<vol2_clause_appendix_a_mdpws_history_service_xmlschema>> for more details).
...
`s12:Envelope/s12:Body/hm:ChangeSequenceReport/@LastReport`:: Indicates the final report of the request; `true` if it is the final report, `false` otherwise.
...
`s12:Envelope/s12:Body/hm:ChangeSequenceReport/hm:ChangeSequence/hm:HistoricLocalizedTexts`:: Sequence of localized texts referenced from within the historic <<acronym_mdib>>. Element is omitted if not requested in the Subscribe request.
`s12:Envelope/s12:Body/hm:ChangeSequenceReport/hm:ChangeSequence/hm:HistoricMdib`:: A <<acronym_mdib>> used as a baseline for subsequent historic reports. Element is omitted if not requested in the Subscribe request.
`s12:Envelope/s12:Body/hm:ChangeSequenceReport/hm:ChangeSequence/hm:HistoricReport`:: A sequence of historic reports.
...
.<<term_history_service>>: Requesting a time range spanning multiple <<acronym_mdib>> sequences
...
.Subscribe message targeted at the <<vol1_spec_sdpi_p_actor_somds_provider>>
...
.SubscribeResponse message targeted at the <<vol1_spec_sdpi_p_actor_somds_consumer>>
...
.ChangeSequenceReport message targeted at the <<vol1_spec_sdpi_p_actor_somds_consumer>>
...
.SubscriptionEnd message targeted at the <<vol1_spec_sdpi_p_actor_somds_consumer>>
