...
The <<term_history_service>> supports hints from the <<vol1_spec_sdpi_p_actor_somds_consumer>>
to indicate the desired entries to reduce the bandwidth required to transmit history reports:

- containment tree entries by `pm:HandleRef`,
- reports types (e.g., `EpisodicAlertReport`, `EpisodicMetricReport`).

NOTE: <<vol1_spec_sdpi_p_actor_somds_provider>>s may ignore hints and transmit all available,
filtered, historic data.
...
[NORMATIVE]
====
A filter in the Dialect designated `urn:oid:1.3.6.1.4.1.19376.1.6.2.10.1.3.1.1` shall contain exactly, and only, one hm:HistoryQuery of type hm:HistoryQueryType.
====

NOTE: The filter may include additional elements and attributes from any namespace.
****
...
[NORMATIVE]
====
For a time range-filtered subscription, a <<vol1_spec_sdpi_p_actor_somds_provider>> shall respond with notifications that reflect the MDIB of the <<vol1_spec_sdpi_p_actor_somds_provider>> and its changes in the closed time interval of ++[hm:HistoryQuery/hm:Range/hm:Time/@StartTime, hm:HistoryQuery/hm:Range/hm:Time/@EndTime]++.
====

[NOTE]
====
* The time interval is a hint.
A <<vol1_spec_sdpi_p_actor_somds_provider>> may respond with data that exceeds, or is a subset of, the interval of ++[hm:HistoryQuery/hm:Range/hm:Time/@StartTime, hm:HistoryQuery/hm:Range/hm:Time/@EndTime]++.

* The term _notifications that reflect the MDIB_ includes localized texts of the LOCALIZATION SERVICE as well.
...
[NORMATIVE]
====
For a version range-filtered subscription, a <<vol1_spec_sdpi_p_actor_somds_provider>> shall respond with notifications that reflect the MDIB of the <<vol1_spec_sdpi_p_actor_somds_provider>> at the given pm:Mdib@SequenceId and pm:Mdib@InstanceId in the closed version interval of ++[hm:HistoryQuery/hm:Range/hm:Version/@StartVersion, hm:HistoryQuery/hm:Range/hm:Version/@EndVersion]++.
====

[NOTE]
====
* A <<vol1_spec_sdpi_p_actor_somds_provider>> may respond with data that exceeds, or is a subset of, the interval of ++[hm:HistoryQuery/hm:Range/hm:Version/@StartVersion, hm:HistoryQuery/hm:Range/hm:Version/@EndVersion]++. For example, if it cannot fully serve the request due to discarded historic information or the filter requests reports from the future.

* The term _notifications that reflect the MDIB_ includes localized texts of the LOCALIZATION SERVICE as well.

* A <<vol1_spec_sdpi_p_actor_somds_provider>> can respond with a subset of data that is not contiguous.
For example, a complete history of waveform streams may not be available.
====
****
...
[NORMATIVE]
====
For an <<term_mdib_sequence_of_changes>> that is requested from a <<vol1_spec_sdpi_p_actor_somds_provider>>, the <<vol1_spec_sdpi_p_actor_somds_provider>> shall send hm:ChangeSequence/hm:HistoricReport elements in ascending order by pm:Mdib@MdibVersion.
====
****
...
[NORMATIVE]
====
A <<vol1_spec_sdpi_p_actor_somds_provider>> shall add the `sdpi:ReportSequence` element to the SOAP header of SOAP messages conveying <<term_history_service>> change sequence reports.
====

NOTE: This requirement satisfies <<r0529>>.
...
| Action uri that identifies the payload of the <<term_history_service>> subscription notification.
| report
...
The XML Schema includes the BICEPS message, participant and extension Model files as provided as an additional resource at https://standards.ieee.org/downloads/.
