...
A <<vol1_spec_sdpi_p_actor_somds_provider>> may respond with data that exceeds, or is a subset of, the interval of ++[hm:TimeRange/@StartTime, hm:TimeRange/@EndTime]++.
...
[NOTE]
====
* A <<vol1_spec_sdpi_p_actor_somds_provider>> may respond with data that exceeds, or is a subset of, the interval of ++[hm:VersionRange/@StartVersion, hm:VersionRange/@EndVersion]++. For example, if it cannot fully serve the request due to discarded historic information or the filter requests reports from the future.
...
.R0528
[sdpi_requirement#r0528,sdpi_req_level=shall,sdpi_req_type=tech_feature]
****
[NORMATIVE]
====
A <<vol1_spec_sdpi_p_actor_somds_provider>> shall add add the `sdpi:ReportSequence` element to the SOAP header of SOAP messages conveying <<term_history_service>> change sequence reports.
====
